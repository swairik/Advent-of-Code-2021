...
void solve() {
    ...
    for(int i = 0; i < (int)start.size(); i++) {
        ...
        else if(start[i].ff + end[i].ss == start[i].ss + end[i].ff) {
            int x = min(start[i].ff, end[i].ff);
            int y = min(start[i].ss, end[i].ss);

            for(; x <= max(start[i].ff, end[i].ff); x++, y++) {
                a[y][x]++;
            }
        }

        else if(start[i].ff + start[i].ss == end[i].ff + end[i].ss) {
            int x = min(start[i].ff, end[i].ff);
            int y = max(start[i].ss, end[i].ss);
            for(; x <= max(start[i].ff, end[i].ff); x++, y--) {
                a[y][x]++;
            }
        }
